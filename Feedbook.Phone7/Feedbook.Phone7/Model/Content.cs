    ...
    public class Content : Entity
    {
        ...
        public static string ToPainText(string contentString)
        {
            ...
            try
            {
                ...
                return ((XElement.Parse(contentString, LoadOptions.PreserveWhitespace)).Value + "").Trim();
            }
            catch
            {
                string clearContent = Regex.Replace(contentString, @"<(.|\n)*?>", string.Empty);
                try { return (HttpUtility.HtmlDecode(clearContent) + "").Trim(); }
                catch { return (clearContent + "").Trim(); }
            }
        }
